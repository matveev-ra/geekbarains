...
public class Cat implements Overcomable {
    ...
    public boolean overcomeObstacle(Obstacle obstacle) {
        ...
        if (obstacle instanceof Treadmill) {
            System.out.println("Кот " + name + " препятствие - " + obstacle.getName() +
    ...
    public boolean run(double length) {
        if (!isTired && length <= runLimit) {
            System.out.println("Кот " + name + " успешно пробежал " + length + "м.");
            return true;
        } else {
            System.out.println("Кот " + name + " не смог пробежать " + length + "м. Он выбывает из гонки.");
            isTired = true;
        }
        return false;
    }
    ...
    public boolean jump(double height) {
        if (!isTired && height <= jumpLimit) {
            System.out.println("Кот " + name + " успешно перепрыгнул " + height + "м.");
            return true;
        } else {
            System.out.println("Кот " + name + " не смог  перепрыгнуть. Он выбывает из гонки.");
            isTired = true;
        }
        return false;
    }
